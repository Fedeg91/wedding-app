...
async function getAdminPhotosHandler(request: Request, { params }: { params: Promise<{ eventSlug: string }> }) {
  ...
  if (parsed.data.sort === "most_liked" && cursor && cursor.likeCount === undefined) return apiError("INVALID_CURSOR", "Malformed pagination cursor", 400);
  const { page, error } = await listAdminPhotos(event.id, { limit: parsed.data.limit, status: parsed.data.status, sort: parsed.data.sort, cursor: cursor ?? undefined }); if (error || !page) return databaseError("admin list photos", error);
