...
const UploadSheet = dynamic(
  () => import("./upload-sheet").then((module) => module.UploadSheet),
  { ssr: false },
);
const ProfileSheet = dynamic(
  () => import("./profile-sheet").then((module) => module.ProfileSheet),
  { ssr: false },
);
...
export function EventGallery({ eventSlug }: { eventSlug: string }) {
  ...
  const [selectedPhoto, setSelectedPhoto] = useState<PhotoFeedItem | null>(
    null,
  );
  ...
  useEffect(() => {
    const update = () => setShowLatest(window.scrollY > 700);
    update();
  ...
  useEffect(() => {
    const unlock = () => {
      try {
        ...
        void context
          .resume()
          .then(() => setAudioReady(context.state === "running"))
          .catch(() => undefined);
      } catch {
        /* Il popup resta disponibile se il browser non supporta Web Audio. */
      }
    };
  ...
  useEffect(
    () => () => {
      if (awardSnoozeTimerRef.current)
        window.clearTimeout(awardSnoozeTimerRef.current);
    },
    [],
  );
  ...
  const registration = useMutation({
    ...
    onSuccess: (createdGuest) => {
      queryClient.setQueryData<{ items: Guest[] }>(
        ["guests", eventSlug],
        (current) => ({
          items: current?.items.some((item) => item.id === createdGuest.id)
            ? current.items
            : [...(current?.items ?? []), createdGuest],
        }),
      );
  ...
  const photosQuery = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) =>
      getPhotoPage(
        eventSlug,
        {
          guestId: guestId === "all" ? undefined : guestId,
  ...
  const readAward = useMutation({
    mutationFn: ({
      ...
      awardId: string;
      action: "dismiss" | "claim";
    }) => respondToGuestAward(eventSlug, guest!.id, awardId, action),
    onSuccess: () =>
      queryClient.setQueryData(["award", eventSlug, guest?.id], {
        award: null,
      }),
  });
  useEffect(() => {
    const awardId = awardQuery.data?.award?.id;
    const context = audioContextRef.current;
    if (
      !awardId ||
    ...
    [659.25, 783.99, 1046.5].forEach((frequency, index) => {
      ...
      gain.gain.exponentialRampToValueAtTime(
        0.0001,
        start + index * 0.12 + 0.3,
      );
      oscillator.connect(gain).connect(context.destination);
      oscillator.start(start + index * 0.12);
      oscillator.stop(start + index * 0.12 + 0.31);
    });
  }, [audioReady, awardQuery.data?.award?.id]);
  const likes = useMutation({
    mutationFn: ({ photoId, liked }: { photoId: string; liked: boolean }) =>
      setPhotoLike(eventSlug, photoId, guest!.id, liked),
    onMutate: async ({ photoId, liked }) => {
      await queryClient.cancelQueries({ queryKey: ["photos", eventSlug] });
      const snapshots = queryClient.getQueriesData<
        InfiniteData<PaginatedResponse<PhotoPost>>
      >({ queryKey: ["photos", eventSlug] });
      queryClient.setQueriesData<InfiniteData<PaginatedResponse<PhotoPost>>>(
        { queryKey: ["photos", eventSlug] },
        (data) => updatePhotoLikeInPages(data, photoId, liked),
      );
      return { snapshots };
    },
  ...
  if (eventQuery.isError)
    return (
      <FullPageMessage
        ...
        message="Non riusciamo a caricare questo evento. Controlla il link o riprova tra poco."
        onRetry={() => void eventQuery.refetch()}
      />
    );

  const event = eventQuery.data;
  if (!event.publicGalleryEnabled)
    return (
      <FullPageMessage
        icon={<Images className="size-10" />}
        title={event.title}
        message="La gallery di questo evento non è più disponibile."
      />
    );
  if (!guest)
    return (
      <GuestOnboarding
        ...
        error={
          registration.isError
            ? "Non è stato possibile registrarti. Riprova."
            : null
        }
      />
    );
  ...
  return (
    <div className="min-h-dvh pb-28">
      ...
      {!online && (
        <div
          className="sticky top-16 z-30 bg-amber-100 px-4 py-2 text-center text-sm font-medium text-amber-900"
          role="status">
          Sei offline. Puoi vedere le foto già caricate, ma serve internet per
      ...
      <main className="mx-auto max-w-2xl space-y-4 py-4 sm:px-4">
        {photosQuery.isPending ? (
          ...
          posts.map((post) => (
            <PhotoCard
              ...
              onLike={(photo) =>
                likes.mutate({
                  photoId: photo.id,
                  liked: !photo.likedByCurrentGuest,
                })
              }
              pendingPhotoId={
                likes.isPending ? likes.variables?.photoId : undefined
              }
              key={post.id}
            />
          ))
          ...
          <div className="px-6 py-24 text-center">
            <Images className="mx-auto mb-3 size-9 text-stone-300" />
            <p className="font-semibold text-stone-700">
              Nessuna foto da mostrare
            </p>
            <p className="mt-1 text-sm text-stone-400">
              {guestId === "all"
                ? "Sii il primo a condividere una foto di oggi."
                : "Prova a cambiare il filtro."}
            </p>
            {guestId === "all" && event.uploadEnabled && (
              <Button className="mt-5" onClick={() => setUploadOpen(true)}>
                <Camera className="size-4" />
                Condividi la prima foto
              </Button>
            )}
          </div>
        )}
        ...
        {photosQuery.isFetchNextPageError && (
          <div className="px-4 py-5 text-center">
            <p className="text-sm text-stone-500">
              Non siamo riusciti a caricare altre foto.
            </p>
            <Button
              className="mt-3"
              variant="outline"
      ...
      {event.uploadEnabled ? (
        <div className="safe-bottom pointer-events-none fixed inset-x-0 bottom-0 z-40 flex justify-center bg-linear-to-t from-[#faf9f7] via-[#faf9f7]/95 to-transparent px-4 pb-4 pt-8">
          <Button
            className="pointer-events-auto min-h-14 w-full max-w-sm text-base shadow-[0_10px_30px_rgba(244,63,94,0.28)]"
        ...
        <div className="safe-bottom fixed inset-x-0 bottom-0 z-40 bg-[#faf9f7]/95 px-4 pb-4 pt-3 text-center text-sm text-stone-500 backdrop-blur">
          Gli sposi hanno disattivato nuovi caricamenti.
        </div>
      )}
      <UploadSheet
        open={uploadOpen}
      ...
      <Dialog
        open={Boolean(
          awardQuery.data?.award && awardQuery.data.award.id !== snoozedAwardId,
        )}
        onOpenChange={(open) => {
          const current = awardQuery.data?.award;
          if (!open && current && !readAward.isPending) {
            setSnoozedAwardId(current.id);
            if (awardSnoozeTimerRef.current)
              window.clearTimeout(awardSnoozeTimerRef.current);
            awardSnoozeTimerRef.current = window.setTimeout(
              () => setSnoozedAwardId(null),
              60_000,
            );
          }
        }}>
        <DialogContent className="text-center">
          <div className="mx-auto flex size-20 items-center justify-center rounded-full bg-amber-100 text-amber-600">
            <Trophy className="size-10" />
          </div>
          <DialogTitle className="mt-4 font-serif text-3xl text-stone-900">
            Congratulazioni! 🎉
          </DialogTitle>
          <DialogDescription className="mt-3 text-lg leading-relaxed text-stone-600">
            {awardQuery.data?.award?.message}
          </DialogDescription>
          <Button
            className="mt-5 w-full"
            disabled={readAward.isPending}
            onClick={() => {
              const current = awardQuery.data?.award;
              if (current)
                readAward.mutate({ awardId: current.id, action: "claim" });
            }}>
            🎁 Sto arrivando a ritirarlo!
          </Button>
        </DialogContent>
      </Dialog>
    </div>
...
function InlineError({
  ...
  return (
    <div className="px-6 py-24 text-center">
      <WifiOff className="mx-auto mb-3 size-9 text-stone-300" />
      <p className="font-semibold text-stone-700">
        {offline ? "Sei offline" : "Impossibile caricare le foto"}
      </p>
      <p className="mt-1 text-sm text-stone-400">
        {offline
          ? "Riconnettiti a internet per aggiornare la gallery."
          : "La connessione sembra instabile. Riprova tra poco."}
      </p>
      <Button
        className="mt-4"
        variant="outline"
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center px-6 text-center">
      <section className="max-w-sm">
        <div className="mx-auto mb-5 flex size-16 items-center justify-center rounded-full bg-rose-50 text-rose-400">
          {icon}
        </div>
        <h1 className="font-serif text-3xl text-stone-800">{title}</h1>
        <p className="mt-3 leading-relaxed text-stone-500">{message}</p>
        {onRetry && (
