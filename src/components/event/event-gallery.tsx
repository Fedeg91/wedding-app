...
export function EventGallery({ eventSlug }: { eventSlug: string }) {
  ...
  return (
    <div className="min-h-dvh pb-28">
      ...
      {event.uploadEnabled ? <div className="safe-bottom pointer-events-none fixed inset-x-0 bottom-0 z-40 flex justify-center bg-gradient-to-t from-[#faf9f7] via-[#faf9f7]/95 to-transparent px-4 pb-4 pt-8"><Button className="pointer-events-auto min-h-14 w-full max-w-sm text-base shadow-[0_10px_30px_rgba(244,63,94,0.28)]" onClick={() => setUploadOpen(true)}><Camera className="size-5" />Aggiungi foto</Button></div> : <div className="safe-bottom fixed inset-x-0 bottom-0 z-40 bg-[#faf9f7]/95 px-4 pb-4 pt-3 text-center text-sm text-stone-500 backdrop-blur">Gli sposi hanno disattivato nuovi caricamenti.</div>}
