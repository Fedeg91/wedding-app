...
import { Camera, CheckCircle2, ImagePlus, Plus, Upload } from "lucide-react";
    ...
    <Dialog open={open} onOpenChange={close}>
      <DialogContent>
        ...
        <div className="mt-5 max-h-[46dvh] space-y-2 overflow-y-auto">
          {items.length === 0 ? <div className="grid grid-cols-2 gap-3"><label className="flex min-h-48 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-rose-200 bg-rose-50/60 p-4 text-center transition hover:bg-rose-50"><Camera className="mb-3 size-8 text-rose-500" /><span className="font-semibold text-stone-800">Scatta una foto</span><span className="mt-1 text-xs text-stone-500">Apri la fotocamera</span><input className="sr-only" type="file" accept="image/*" capture="environment" onChange={selectImages} /></label><label className="flex min-h-48 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-stone-200 bg-stone-50 p-4 text-center transition hover:bg-stone-100"><ImagePlus className="mb-3 size-8 text-stone-500" /><span className="font-semibold text-stone-800">Dalla galleria</span><span className="mt-1 text-xs text-stone-500">Fino a 4 immagini</span><input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={selectImages} /></label></div> : <>{items.map((item) => <UploadItem key={item.id} item={item} onRemove={() => removeItem(item.id)} onRetry={() => setItems((current) => current.map((entry) => entry.id === item.id ? { ...entry, status: "queued", error: undefined } : entry))} />)}{items.length < MAX_BATCH_SIZE && !busy && <div className="grid grid-cols-2 gap-2"><label className="flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-full border border-dashed border-rose-300 text-sm font-semibold text-rose-600 hover:bg-rose-50"><Camera className="size-4" />Scatta<input className="sr-only" type="file" accept="image/*" capture="environment" onChange={selectImages} /></label><label className="flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-full border border-dashed border-stone-300 text-sm font-semibold text-stone-600 hover:bg-stone-50"><Plus className="size-4" />Galleria<input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={selectImages} /></label></div>}</>}
        </div>
