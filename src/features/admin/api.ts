...
export function getAdminPhotos(eventSlug: string, status: "all" | "published" | "hidden", cursor?: string) { const query = new URLSearchParams({ limit: "30", status }); if (cursor) query.set("cursor", cursor); return fetchJson<PaginatedResponse<AdminPhoto>>(`/api/admin/events/${encodeURIComponent(eventSlug)}/photos?${query}`); }
