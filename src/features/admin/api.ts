...
import type { AdminGuestAward, GuestAward } from "@/features/awards/server/award-repository";
...
export function getAdminAwards(eventSlug: string) { return fetchJson<{ items: AdminGuestAward[] }>(`/api/admin/events/${encodeURIComponent(eventSlug)}/awards`); }
export function deliverGuestAward(eventSlug: string, awardId: string) { return updateGuestAward(eventSlug, awardId, "deliver"); }
export function resendGuestAward(eventSlug: string, awardId: string) { return updateGuestAward(eventSlug, awardId, "resend"); }
function updateGuestAward(eventSlug: string, awardId: string, action: "deliver" | "resend") { return fetchJson<{ updated: boolean }>(`/api/admin/events/${encodeURIComponent(eventSlug)}/awards`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ awardId, action }) }); }
