...
export function deliverGuestAward(eventSlug: string, awardId: string) { return fetchJson<{ delivered: boolean }>(`/api/admin/events/${encodeURIComponent(eventSlug)}/awards`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ awardId }) }); }
