...
type PhotoRow = { id: string; post_id: string; post_created_at: string; group_position: number; mock_image_url: string | null; cloudinary_public_id: string | null; width: number | null; height: number | null; caption: string | null; created_at: string; guest_id: string; guest_nickname: string; guest_avatar_key: import("@/features/guests/avatars").AvatarId; like_count: number; liked_by_current_guest: boolean };
...
export async function listPhotos(eventId: string, options: FeedOptions): Promise<{ page: PaginatedResponse<PhotoPost> | null; error: unknown }> {
  ...
  const items = visibleGroups.flatMap(([postId, groupRows]): PhotoPost[] => {
    const photos = groupRows.flatMap((row): PhotoFeedItem[] => {
      ...
      return imageUrl && fullscreenUrl ? [{ id: row.id, imageUrl, fullscreenUrl, width: row.width, height: row.height, caption: row.caption, createdAt: row.created_at, likeCount: Number(row.like_count), likedByCurrentGuest: row.liked_by_current_guest, guest: { id: row.guest_id, nickname: row.guest_nickname, avatarKey: row.guest_avatar_key } }] : [];
    });
    const first = groupRows[0];
    return photos.length ? [{ id: postId, createdAt: first.post_created_at, caption: first.caption, guest: { id: first.guest_id, nickname: first.guest_nickname, avatarKey: first.guest_avatar_key }, photos }] : [];
  });
...
export async function createPublishedPhoto(input: CreatePhotoInput): Promise<{ item: PhotoFeedItem | null; error: unknown }> {
  ...
  const { data: existing, error: existingError } = await measureDatabase("photos.find_idempotent", () => client.from("photos").select("id, event_id, guest_id, cloudinary_public_id, width, height, caption, created_at, guests!inner(id, nickname, avatar_key)").eq("event_id", input.eventId).or(`client_upload_id.eq.${input.clientUploadId},cloudinary_public_id.eq.${input.cloudinaryPublicId}`).maybeSingle());
  ...
  if (existing) {
    ...
    const rawGuest = existing.guests as unknown as { id: string; nickname: string; avatar_key: import("@/features/guests/avatars").AvatarId };
    const joinedGuest = { id: rawGuest.id, nickname: rawGuest.nickname, avatarKey: rawGuest.avatar_key };
